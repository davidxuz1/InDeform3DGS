...
class EndoNeRF_Dataset(object):
    def __init__(
        ...
        downsample=1.0,
        test_every=8,
        img_width=None,  # Nuevo parámetro para ancho  #edit own_data
        img_height=None  # Nuevo parámetro para alto #edit own_data
    ):
        
        # Usar dimensiones proporcionadas o detectar automáticamente del dataset # edit own_data
        sample_img = Image.open(sorted(glob.glob(os.path.join(datadir, "images", "*.png")))[0])
        self.img_wh = (
            img_width if img_width else int(sample_img.size[0] / downsample),
            img_height if img_height else int(sample_img.size[1] / downsample)
        )
        print(f'Image size is {self.img_wh}')
        # self.img_wh = (
        #     int(640 / downsample),
        #     int(512 / downsample),
        # )
        ...
        # self.transform = T.ToTensor()
            ...
            # Asegurar que el transform use las dimensiones correctas # edit own_data
        self.transform = T.Compose([
        T.Resize((self.img_wh[1], self.img_wh[0])),
        T.ToTensor()
        ])
    ...
    def format_infos(self, split):
        ...
        for idx in tqdm(idxs):
            ...
            # Convertir numpy array a PIL Image antes de aplicar transform # edit own_video
            ...
            # Convertir a PIL Image antes de transform
            color_pil = Image.fromarray((color * 255).astype(np.uint8))
            image = self.transform(color_pil)
    ...
    def get_pts_cam(self, depth, mask, color, disable_mask=False):
        ...
        # Asegurarse de que depth sea 2D  # own_video
        if len(depth.shape) == 3:
            depth = depth[:,:,0]  # Tomar solo el primer canal
